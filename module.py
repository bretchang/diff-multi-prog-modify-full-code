...
diff_map_queue = queue.Queue()
# 鎖
lock = threading.Lock()


class Program:
    def __init__(self, equipment, chamber, recipe, starttime, endtime, training_starttime, training_endtime, e):
        if e.key == True:
            print("登入成功")
            self.eqip = equipment
            ...
            self.training_starttime = training_starttime
            self.training_endtime = training_endtime
            self.e = e
        else:
            print("登入失敗")
    ...
    @staticmethod
    def data_process_diff(dataset):
        # recipe step 若為"" 則以TIME取代
        # dataset["recipe_step"] = dataset["recipe_step"].apply(
            #lambda x: "TIME" if x == "" else str(x))
        dataset["feature_name"] = dataset["parameter"] + "__" + dataset["recipe_step"] + "__" + dataset[
            "statistics_name"]
        dataset = dataset[["start_time", "wafers",
                           "recipe", "feature_name", "statistics_value"]]
        ...
        dataset.columns = [i[0] if i[1] == "" else i[1]
                           for i in dataset.columns]

        # 管子內空位置移除
        dataset = dataset[~dataset["Alias"].str.startswith('[{"alias_id":""')]

        map = dataset.groupby(["DateTime"])["Alias"].aggregate(
            [Program.handle_tube_wafers]).reset_index()
        map["handle_tube_wafers"] = map["handle_tube_wafers"].apply(
            lambda x: [i["alias_id"] for i in x])
        try:
            map["Alias"] = map["handle_tube_wafers"].apply(
                lambda x: re.search("K\w{10}", str(x)).group(0))
        except:
            map["Alias"] = map["handle_tube_wafers"].apply(lambda x: x[0])

        dataset = dataset.drop_duplicates(subset=["DateTime"])
        dataset = pd.merge(map[["DateTime", "Alias"]], dataset.drop(
            "Alias", axis=1), on="DateTime", how="inner")
        map = map[["Alias", "handle_tube_wafers"]]
        ...
        for recipe_name in self.recipe:
            ...
            try:
                ...
                if rows and len(rows) > 0:
                    ...
                    data, map = Program.data_process_diff(dataset)
                    with lock:
                        data_que.put(data)
                        diff_map_queue.put(map)
            except Exception as e:
    ...
    def merge_list():
        ...
        data_all = data_all.sort_values(
            ...
            drop=True)
        diff_map = pd.DataFrame()
        while not diff_map_queue.empty():
            sub = diff_map_queue.get()
            diff_map = pd.concat([diff_map, sub], ignore_index=True)
        return data_all, diff_map
    ...
    def query(self):
        ...
        data_all, self.diff_map = Program.merge_list()  # diff_map
    ...
    def output_raw_data(self):
        ...
        self.diff_map.to_csv("tube_mapping.csv", index=False)
    ...
    def run(self, training_starttime, training_endtime):
        ...
        data = self.fdc_process[(self.fdc_process["DateTime"] >= training_starttime) & (
            self.fdc_process["DateTime"] <= training_endtime)].reset_index(drop=True)
        rm = data.iloc[:, 3:].std(
        ...
        path = os.path.join(
            "%s %s %s" % (time.strftime("%Y%m%d%H%M%S",
                          time.localtime()), self.eqip, self.chamber),
            "multProg")
        ...
        metrics = {"Diff Multi_Prog": 0}
        artifacts = [path]
        artifacts_show = {}
        for class_ in self.map["GroupName"].unique():
            artifacts_show[class_] = os.path.join(path, class_, "t2", class_ + ".png")
